...
class OneModel(nn.Module):
    ...
    def forward(self, query_rgb, support_rgb, support_mask,label):
        ...
        support_mask_attentive= F.interpolate(support_mask, support_feature.shape[-2:], mode='bilinear',
                                              align_corners=True)
        class_vector = self.Weighted_GAP(support_feature,support_mask_attentive)
        b,c,h,w = support_feature.shape
        # class_vector = class_vector.view(b,c)
        query_attentive = self.narrow_gap(query_feature,class_vector)
        class_label = self.fc_classify(query_attentive.mean(dim=[2,3]).view(b,c))
        # meta_vector = self.fc_2(F.relu(self.fc_1(class_vector)))
        
        # if self.training :
        #     inds_x,inds_y = np.where(label.cpu().numpy()==1)
        #     query_weights = self.fc_1.weight[inds_y,:]
        #     b,c,h,w = query_feature.shape
        #     query_weights = query_weights.view(b,c,1,1)
        #     # query_weights = torch.mm(label.float(),self.fc_1.weight.float())
        #     # query_weights = query_weights.view(b,c,1,1)
        #     seed_map = query_feature*query_weights
        #     seed_map = seed_map.sum(dim=1).view(b,1,h,w)
        #     query_feature = torch.cat([query_feature,seed_map],dim=1)
        #     query_feature = self.trans2(query_feature)
        #     class_vector1 = query_feature.mean(dim=[2, 3], keepdim=True).view(b,c)
        #     class_label1 = self.fc_1(class_vector1)
        # else:
        #     min_value,_ = class_label.min(dim=1)
        #     class_label = class_label-min_value
        #     max_value,_ = class_label.max(dim=1)
        #     class_label = class_label/max_value
        #     # class_label = F.sigmoid(class_label)
        #     b,c,h,w = query_feature.shape
        #     query_weights = torch.mm(class_label,self.fc_1.weight)/class_label.sum(dim=1)
        #     query_weights = query_weights.view(b,c,1,1)
        #     seed_map = query_feature*query_weights
        #     seed_map = seed_map.sum(dim=1).view(b,1,h,w)
        #     query_feature = torch.cat([query_feature,seed_map],dim=1)
        ...
        for i in range(self.num_pro):
            ...
            vec = vec_pos[i]
            query_feature_i= self.narrow_gap(query_feature,vec)
            # class_vector_i = query_feature_i.mean(dim=[2,3])
            
            exit_feat_in_ = self.f_v_concate(query_feature_i, vec, feature_size)
            exit_feat_in_ = self.layer55(exit_feat_in_)
            if i == 0:
                exit_feat_in = exit_feat_in_
                # class_vector = class_vector_i
            else:
                exit_feat_in = exit_feat_in + exit_feat_in_
                # class_vector = class_vector + class_vector_i
        ...
        return support_feature, query_feature, vec_pos, out, class_label
